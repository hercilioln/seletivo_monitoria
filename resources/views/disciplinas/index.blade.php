...
<div class="d-flex justify-content-end mb-3">
    <a href="{{ route('disciplinas.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Adicionar Disciplina</a>
</div>
        ...
        <div class="row">
          <div class="col-sm-12">
            <table id="example" class="table table-bordered hover dataTable dtr-inline" aria-describedby="example1_info">
                ...
                <tbody>
                    @foreach ($disciplinas as $items)
                    <tr>
                        <td class="align-middle">{{$items->nome}}</td>
                        <td class="text-right align-middle">
                            <!-- Botão de editar -->
                            <a href="{{ route('disciplinas.edit', $items->id) }}" class="btn btn-sm btn-warning mr-2"><i class="fa fa-edit"></i> Editar</a>
                            <!-- Botão de excluir -->
                            <form action="{{ route('disciplinas.destroy', $items->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
